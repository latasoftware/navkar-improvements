...
public partial class Sugar_Inword_pgeRawanagiBook : System.Web.UI.Page
{
    ...
    {
        Doc_date = clsCommon.getString("select ifnull(max(Date_Format(Doc_Date,'%Y/%m/%d')),0) as Date from rawangiline_head ");
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (btnSave.Text == "Save")
        {
            ...
            for (int i = 0; i < grdDetail.Rows.Count; i++)
            {
                ...
                try
                {
                    mc = Convert.ToInt32(clsCommon.getString("select ifnull(accoid,0) as id from qrymstaccountmaster where Ac_Code=" + Mill_Code + " " +
        ...
        try
        {
            ...
            if (hdnfClosePopup.Value == "txtMILL_CODE")
            {
                ...
                string qry = "select Ac_Code,Ac_Name_E,CityName from qrymstaccountmaster where Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " " +
                    " and " + name + " and Ac_type='M' order by Ac_Name_E";
                this.showPopup(qry);
            }
            ...
            if (hdnfClosePopup.Value == "txtTransport_code")
            {
                ...
                string qry = "select Ac_Code,Ac_Name_E,CityName from qrymstaccountmaster where Company_Code=" + Convert.ToInt32(Session["Company_Code"].ToString()) + " " +
                    " and " + name + " and Ac_type='T' order by Ac_Name_E";
                this.showPopup(qry);
            }
